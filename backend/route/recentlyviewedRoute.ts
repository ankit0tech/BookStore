        ...
        if(req.userId) {
            ...
            const recently_viewed = await prisma.$transaction(async (prisma) => {
                ...
                const newRecentlyViewed = await prisma.recently_viewed.upsert({
                    where: {
                        user_id_book_id: {user_id: userId, book_id: Number(id)}
                    },
                    update: {
                        updated_at: new Date()
                    },
            ...
            if (!recently_viewed) {
                logger.error(`Error while adding item to recently viewed for user: ${req.authEmail}`);
                return res.status(400).json({ message: 'Error while adding item to recently viewed'});
            }
            
            logger.info(`Successfully added recently viewed item for user: ${req.authEmail}`);
...
router.get('/', authMiddleware, async (req: Request, res: Response) => {
    try {
        ...
        if(!items) {
            logger.error(`Error while fetching recently viewed items for user: ${req.authEmail}`);
            return res.status(400).json({ message: 'Error while fetching recently viewed items'});

        }
        logger.info(`Retrieved recently viewed items for user: ${req.authEmail}`);
...
router.delete('/remove/:id(\\d+)', authMiddleware, async (req: Request, res: Response) => {
    try {
        ...
        const removedItem = await prisma.recently_viewed.delete({
            where: {
                user_id_book_id: {user_id: req.userId, book_id: Number(id)}
            }
        });

        if(!removedItem) {
            logger.error(`Error while removing item from recently viewed for user: ${req.authEmail}`);
            return res.status(400).json({ message: 'Error while removing item from recently viewed'});

        }
        logger.info(`Successfully removed item from recently viewed for user: ${req.authEmail}`);
