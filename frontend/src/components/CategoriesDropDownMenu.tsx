...
const CategoriesDropDownMenu = <T extends string> ({title, defaultValue, selectedOptionStatus, setSelectedOptionStatus, categories, getLabel}: DropDownMenuProps<T>) => {
    ...
    }, [categories, selectedOptionStatus, getLabel]);
            ...
            >
                ...
                {categories?.map((category) => (
                    <div
                        className="w-full text-nowrap cursor-pointer px-4 py-1.5 box-border hover:text-gray-950 hover:bg-gray-100 rounded-lg transition-colors duration-200"
                        key={category.id}
                        onClick={() => {setSelectedOptionStatus(String(category.id)); setShowDropdown(false)}}
                    >
                        {getLabel ? getLabel(category.title) : category.title}
                    </div>
                ))}
            </div>
        </div>
